...
var run = function(argv) {
	...
	jslinker.makeTempDir()
		...
		.then(function(pathConfig) {
			var packageJson;
			try {
				packageJson = fs.readJsonSync(path.join(pathConfig.appPath, "package.json"));
			} catch (err) {
				if (err.code == "ENOENT") {
					throw new error.LinkerError("File 'package.json' could not be found", err);
				} else {
					throw err;
				}
			}
			...
			if (!startScript) {
				throw new error.LinkerError("No start script found, please specify by command line parameter --main");
			}
			...
			console.error();
			if (err instanceof error.LinkerError) {
