...
var resolveDependencies = function(path) {
	...
	return new Promise(function(resolve, reject) {
		var npm = spawn("npm", ["install", "--production"], {
			cwd: path,
			stdio: "inherit"
		});
...
var getPackage = function(npm, tempPath) {
	console.log("Load package", npm);
	return new Promise(function(resolve, reject) {
		var npmPath = path.join(tempPath, "npm");

		fs.ensureDir(npmPath, function(err) {
			if (err) {
				return reject(err);
			}

			var npmApp = spawn("npm", ["install", npm, "--production"], {
				cwd: npmPath,
				stdio: "inherit"
			});

			npmApp.on("close", function(code) {
				if (code !== 0) {
					return reject("npm failed");
				}

				resolve(path.join(npmPath, "node_modules", npm));
			});
		});
	});
};
